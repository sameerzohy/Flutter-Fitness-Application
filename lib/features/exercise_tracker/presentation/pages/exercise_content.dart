...
class ExerciseContent extends StatefulWidget {
  ...
  const ExerciseContent({super.key, required this.scrollController});
...
class _ExerciseContentState extends State<ExerciseContent> {
  ...
  void initState() {
    ...
    print(
      'ExerciseContent: initState - Initial currentUser: ${_currentUser?.id ?? "null"}',
    );
    ...
    if (_currentUser != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        print(
          'ExerciseContent: initState - Dispatched FetchAllWorkoutsEvent for user: ${_currentUser!.id}',
        );
      });
    } else {
      print(
        'ExerciseContent: initState - User not logged in, no FetchAllWorkoutsEvent dispatched.',
      );
    }
    ...
    _supabaseClient.auth.onAuthStateChange.listen((data) {
      if (mounted) {
        setState(() {
          ...
          print(
            'ExerciseContent: Auth State Changed - New currentUser: ${_currentUser?.id ?? "null"}',
          );
          if (_currentUser != null) {
            context.read<ScheduledWorkoutBloc>().add(
              FetchAllWorkoutsEvent(_currentUser!.id),
            ...
            print(
              'ExerciseContent: Auth State Changed - Re-dispatched FetchAllWorkoutsEvent for user: ${_currentUser!.id}',
            );
          } else {
            _scheduledWorkouts = []; // Clear workouts if user logs out
            print(
              'ExerciseContent: Auth State Changed - User logged out, cleared workouts.',
            );
          }
        });
      }
        ...
        children: [
          ...
          BlocConsumer<ScheduledWorkoutBloc, ScheduledWorkoutState>(
            listener: (context, state) {
              if (state is ScheduledWorkoutsDisplaySuccess) {
                setState(() {
                  ...
                  print(
                    'ExerciseContent Listener: Received ScheduledWorkoutsDisplaySuccess. Total workouts: ${_scheduledWorkouts.length}',
                  );
                  for (var i = 0; i < _scheduledWorkouts.length; i++) {
                    final workout = _scheduledWorkouts[i];
                    print(
                      '  Scheduled Workout [$i]: Title: ${workout.title}, DateTime: ${workout.dateTime}',
                    );
                  }
                });
                ...
                print(
                  'ExerciseContent Listener: ScheduledWorkoutFailure: ${state.message}',
                );
              } else if (state is ScheduledWorkoutLoading) {
                print('ExerciseContent Listener: ScheduledWorkoutLoading...');
              }
              if (state is ScheduledWorkoutSuccess ||
                  state is ScheduledWorkoutDeleteSuccess) {
                print(
                  'ExerciseContent Listener: Workout CRUD success, re-fetching workouts.',
                );
                if (_currentUser != null) {
                  context.read<ScheduledWorkoutBloc>().add(
                    FetchAllWorkoutsEvent(_currentUser!.id),
            ...
            builder: (context, state) {
              print(
                'ExerciseContent Builder: Building UI. Current Bloc State: $state',
              );
              print(
                'ExerciseContent Builder: _currentUser is ${_currentUser?.id ?? "null"}',
              );
              print(
                'ExerciseContent Builder: _scheduledWorkouts length: ${_scheduledWorkouts.length}',
              );

              if (state is ScheduledWorkoutLoading &&
                  _scheduledWorkouts.isEmpty) {
              ...
              if (_currentUser == null) {
                print(
                  'ExerciseContent Builder: User not logged in. Displaying login message.',
                );
                return Text(
              ...
              final sortedWorkouts = List<ScheduledWorkout>.from(
                _scheduledWorkouts,
              )..sort((a, b) => b.dateTime.compareTo(a.dateTime));
              print(
                'ExerciseContent Builder: Found ${sortedWorkouts.length} total workouts.',
              );

              final workoutsToShow = sortedWorkouts.take(2).toList();
              print(
                'ExerciseContent Builder: Displaying ${workoutsToShow.length} newest workouts.',
              );
