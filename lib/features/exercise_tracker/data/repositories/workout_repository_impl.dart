...
class WorkoutRepositoryImpl implements WorkoutRepository {
  ...
  @override
  Future<Either<Failure, List<WorkoutHistoryEntity>>> loadWorkoutHistory(String exerciseId) async {
    try {
      ...
      final remoteHistory = await remoteDataSource.loadWorkoutHistory(exerciseId);
  ...
  @override
  Future<Either<Failure, Unit>> saveWorkoutHistory(WorkoutHistoryEntity workout) async {
    try {
      ...
      await localDataSource.saveWorkoutHistory(workout.exerciseId, workoutModel);
  ...
  @override
  Future<Either<Failure, Unit>> updateWorkoutHistory(WorkoutHistoryEntity workout) async {
    try {
      final workoutModel = WorkoutHistoryModel(
        id: workout.id,
        userId: workout.userId,
      ...
      await localDataSource.updateWorkoutHistory(workout.exerciseId, workoutModel);
...
@override
  Future<Either<Failure, List<WorkoutHistoryEntity>>> loadAllWorkoutHistory() async {
    try {
  ...
  @override
  Future<Either<Failure, Unit>> deleteWorkoutHistory(String exerciseId, String workoutId) async {
    try {
      await localDataSource.deleteWorkoutHistory(exerciseId, workoutId);
      await remoteDataSource.deleteWorkoutHistory(workoutId); // Remote only needs workoutId
      return Right(unit);
    } on Failure catch (e) {
      return Left(Failure(message: e.message));
    } on Failure catch (e) {
