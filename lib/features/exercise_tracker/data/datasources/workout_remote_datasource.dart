...
abstract class WorkoutRemoteDataSource {
  ...
  Future<Unit> updateWorkoutHistory(WorkoutHistoryModel workout); // Added update
  Future<Unit> deleteWorkoutHistory(String workoutId); // Added delete individual
...
class WorkoutRemoteDataSourceImpl implements WorkoutRemoteDataSource {
  ...
  WorkoutRemoteDataSourceImpl({required this.supabaseClient, required this.uuid});
    ...
    try {
      ...
      if (response != null) {
        return (response as List)
            .map((json) => WorkoutHistoryModel.fromMap(json as Map<String, dynamic>))
            .toList();
      }
      return [];
    } on PostgrestException catch (e) {
      throw Failure(message: 'Supabase error loading workout history: ${e.message}');
    } catch (e) {
      throw Failure(message: 'Failed to load workout history from Supabase: $e');
    }
  }
 ...
 @override
  Future<List<WorkoutHistoryModel>> loadAllWorkoutHistory() async {
    try {
      ...
      if (response != null) {
        return (response as List)
            .map((json) => WorkoutHistoryModel.fromMap(Map<String, dynamic>.from(json)))
            .toList();
      }
      return [];
    } on PostgrestException catch (e) {
      throw Failure(message: 'Supabase error loading all workout history: ${e.message}');
    } catch (e) {
      throw Failure(message: 'Failed to load all workout history from Supabase: $e');
    }
  }
  @override
  Future<Unit> saveWorkoutHistory(WorkoutHistoryModel workout) async {
    try {
      ...
      final workoutToSave = workout.copyWith(
        userId: userId,
        id: workout.id ?? uuid.v4(), // Generate ID if not present
      );

      await supabaseClient.from('workout_history').insert(workoutToSave.toMap());
      return unit;
    } on PostgrestException catch (e) {
      throw Failure(message: 'Supabase error saving workout history: ${e.message}');
    } catch (e) {
  ...
  Future<Unit> updateWorkoutHistory(WorkoutHistoryModel workout) async {
    try {
      ...
      throw Failure(message: 'Supabase error updating workout history: ${e.message}');
    } catch (e) {
      throw Failure(message: 'Failed to update workout history in Supabase: $e');
    }
  }

  @override
  Future<Unit> deleteWorkoutHistory(String workoutId) async { // Added delete individual
    try {
      await supabaseClient
          .from('workout_history')
          .delete()
      ...
      throw Failure(message: 'Supabase error deleting workout history: ${e.message}');
    } catch (e) {
      throw Failure(message: 'Failed to delete workout history from Supabase: $e');
    }
  }
  ...
  Future<Unit> clearWorkoutHistory(String exerciseId) async {
    try {
      ...
      throw Failure(message: 'Supabase error clearing workout history: ${e.message}');
    } catch (e) {
      throw Failure(message: 'Failed to clear workout history from Supabase: $e');
    }
  }
}
